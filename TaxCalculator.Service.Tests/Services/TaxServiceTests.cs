...
namespace TaxCalculatorService.Services.Tests
{
    [TestClass()]
    public class TaxServiceTests
    {
        ...
        public void GetLocationTaxRatesTest()
        {
            ...
            var rates = service.GetLocationTaxRates("33029");
          
            Assert.AreEqual("33029", rates.Zip);
            Assert.AreEqual("FL", rates.State);
            Assert.AreEqual(0.06m, rates.StateRate);
            Assert.AreEqual("BROWARD", rates.County);
            Assert.AreEqual(0.01m, rates.CountyRate);
            Assert.AreEqual("HOLLYWOOD", rates.City);
            Assert.AreEqual(0m, rates.CityRate);
            Assert.AreEqual(0.0m, rates.CombinedDistrictRate);
            Assert.AreEqual(0.07m, rates.CombinedRate);
        ...
        public void CalculateOrderTaxesTest()
        {
            ...
            var rates = service.CalculateOrderTaxes(new
            {
                from_country = "US",
                from_zip = "92093",
                from_state = "CA",
